...
export const RecordForm: React.FC<Props> = ({ onSave, initialRecord, onCancelEdit }) => {
  const [date, setDate] = useState(initialRecord?.date ?? today());
  const [bedTime, setBedTime] = useState(initialRecord?.bedTime ?? '23:00');
  const [wakeTime, setWakeTime] = useState(initialRecord?.wakeTime ?? '07:00');
  const [quality, setQuality] = useState<1 | 2 | 3 | 4 | 5>(initialRecord?.quality ?? 3);
  const [notes, setNotes] = useState(initialRecord?.notes ?? '');
  const [tags, setTags] = useState<SleepTag[]>(initialRecord?.tags ?? []);
  const [saved, setSaved] = useState(false);

  React.useEffect(() => {
    setDate(initialRecord?.date ?? today());
    setBedTime(initialRecord?.bedTime ?? '23:00');
    setWakeTime(initialRecord?.wakeTime ?? '07:00');
    setQuality(initialRecord?.quality ?? 3);
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>{initialRecord ? '記録を編集しましょう' : '昨夜の睡眠を記録しましょう'}</p>
      </header>
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <button type="submit" className={styles.submitBtn}>
          {saved ? '✅ 保存しました！' : initialRecord ? '変更を保存する' : '記録を保存する'}
        </button>
        {initialRecord && onCancelEdit && (
          <button type="button" className={styles.submitBtn} onClick={onCancelEdit}>
            編集をキャンセル
          </button>
        )}
      </form>
